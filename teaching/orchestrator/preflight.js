...
function normalizeCheckResult(result, id) {
  ...
  if (typeof result !== 'object' || Array.isArray(result)) {
    throw new TypeError(`Preflight check ${id} must return boolean or an object.`);
  }
  return Object.freeze({
    ok: result.ok === true,
    reason: result.reason == null ? `PREFLIGHT_REJECTED:${id}` : String(result.reason),
    safeMetadata: Object.freeze({ ...(result.safeMetadata || {}) }),
  });
}

function createOrchestratorPreflight({
  checks = [],
  resolveRequiredCheckIds = async () => [],
  ...
  for (const check of checks) {
    if (!check || typeof check.id !== 'string' || typeof check.evaluate !== 'function') {
      throw new TypeError('Every Teaching D05 preflight check requires id and evaluate().');
    }
    if (checkById.has(check.id)) throw new TypeError(`Duplicate Teaching preflight check: ${check.id}`);
  ...
  async function run({ envelope, authoritativeSnapshot, requestContext = {} } = {}) {
    ...
    assertCapabilityBinding(capability.id, {
      authorityLevel: envelope.capability.authority_ceiling,
      authoritativeOwnerBoundary: envelope.capability.authoritative_owner_boundary,
      promptFamilyId: capability.prompt_family_id,
    });

    if (await isCapabilityEnabled(capability, requestContext) !== true) {
      const error = new Error(`Teaching capability is disabled or unavailable: ${capability.id}`);
      error.code = 'TEACHING_D05_CAPABILITY_DISABLED';
      throw error;
    }
    ...
    if (capability.authority_ceiling !== 'T0') {
      ...
      if (comparison.stale || preconditions.stale) {
        const error = new Error(`Preflight authoritative state changed: ${reasons.join(', ')}`);
        error.code = 'TEACHING_D05_PREFLIGHT_STATE_STALE';
        error.reasons = Object.freeze(reasons);
        throw error;
      }
    }
    ...
    const uniqueRequired = [...new Set(requiredIds.map((value) => String(value).trim()).filter(Boolean))];
    const passed = [];
    for (const id of uniqueRequired) {
      const check = checkById.get(id);
      if (!check) {
        const error = new Error(`Required Teaching preflight gate is not registered: ${id}`);
        error.code = 'TEACHING_D05_REQUIRED_PREFLIGHT_GATE_MISSING';
        throw error;
      }
      const result = normalizeCheckResult(
        await check.evaluate({ envelope, authoritativeSnapshot, requestContext }),
        check.id
      );
      if (!result.ok) {
        const error = new Error(result.reason);
        error.code = 'TEACHING_D05_PREFLIGHT_REJECTED';
        error.checkId = check.id;
        error.safeMetadata = result.safeMetadata || {};
        throw error;
      }
