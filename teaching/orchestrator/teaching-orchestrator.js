...
function createTeachingOrchestrator({promptControl,aiAdapter,executionStore,stateReader,contextAssembler,preflight,ownerRouter,deterministicHandlers={},cancellationReader=async()=>false,randomUUID,clock=()=>new Date()}={}) {
  ...
  async function execute(request={}) {
    ...
    try {
      ...
      const modelResult=await aiAdapter.execute({invocation,academicInput:request.academicInput||{},schemaValidator:request.schemaValidator,domainValidator:request.domainValidator,provenanceValidator:request.provenanceValidator,deterministicChecks:request.deterministicChecks||[],validationContext:request.validationContext||{},safeCommunicationFallback:request.safeCommunicationFallback||null});
      if(!modelResult.accepted){const policy=authorityFailurePolicy(capability.authority_ceiling);await safeMark(executionId,'NOOP',{validationOutcome:modelResult.fallbackUsed?'SAFE_FALLBACK':'REJECTED',failureCode:policy.disposition});return Object.freeze({executionId,replay:false,accepted:false,fallbackUsed:modelResult.fallbackUsed===true,fallback:modelResult.fallback,failureDisposition:policy.disposition,authoritativeMutationPerformed:false});}
