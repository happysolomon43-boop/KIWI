...
const FORBIDDEN_AUDIT_KEYS = Object.freeze(new Set([
  'prompt','prompt_text','raw_prompt','raw_response','model_response','chain_of_thought',
  'hidden_reasoning','reasoning_trace','private_scratchpad','protected_payload',
]));
...
function sanitizeAuditMetadata(value = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('auditMetadata must be an object.');
  const safe = {};
  ...
  if (Buffer.byteLength(JSON.stringify(safe), 'utf8') > 32_768) fail('auditMetadata exceeds the 32 KiB safe metadata limit.');
  return freezeDeep(safe);
}
function normalizeStateReference(value, { required = true } = {}) {
  if (value == null && !required) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('stateReference must be an object.');
  return freezeDeep({
...
function normalizePreconditions(value = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('preconditions must be an object.');
...
function normalizeOrchestrationTrigger(trigger, { serverNow = new Date() } = {}) {
  if (!trigger || typeof trigger !== 'object' || Array.isArray(trigger)) fail('Teaching orchestration trigger must be an object.');
  ...
  if (type === ORCHESTRATION_TRIGGER_TYPES.SCHEDULED_DUE_EVENT && !dueAt) fail('scheduled_due_event trigger requires due_at.');
  const isClientInput = type === ORCHESTRATION_TRIGGER_TYPES.AUTHENTICATED_INPUT;
  const authoritativeOccurredAt = isClientInput ? serverReceivedAt : suppliedOccurredAt == null ? serverReceivedAt : toIso(suppliedOccurredAt, 'trigger.occurred_at');
  return freezeDeep({
    type,
    ...
    client_occurred_at: isClientInput && suppliedOccurredAt != null ? toIso(suppliedOccurredAt, 'trigger.occurred_at') : null,
...
function normalizeResultContract(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('resultContract must be an object.');
  return freezeDeep({
...
function createExecutionEnvelope({executionId,trigger,capabilityId,declaredAuthorityLevel=null,stateReference=null,preconditions={},provenanceRefs=[],resultContract,correlationId,causationId=null,idempotencyKey=null,deadlineAt=null,auditMetadata={},serverNow=new Date()}={}) {
  ...
  if (declaredAuthorityLevel != null) {
    ...
    if (declared !== capability.authority_ceiling) fail(`${capability.id} must execute at its registered authority ceiling ${capability.authority_ceiling}.`, 'TEACHING_D05_AUTHORITY_MISMATCH');
  }
  ...
  const normalizedState = normalizeStateReference(stateReference, { required: capability.authority_ceiling !== 'T0' });
  const promptContract = capability.authority_ceiling === 'T0' ? null : freezeDeep({
  ...
  return freezeDeep({
    ...
    capability: {id:capability.id,execution_class:capability.execution_class,authority_ceiling:capability.authority_ceiling,authoritative_owner_boundary:capability.authoritative_owner_boundary,commit_posture:capability.commit_posture},
...
module.exports={ORCHESTRATION_TRIGGER_TYPES,TRIGGER_AUTHORITY_CATEGORY,FORBIDDEN_AUDIT_KEYS,normalizeOrchestrationTrigger,normalizeStateReference,normalizePreconditions,sanitizeAuditMetadata,createExecutionEnvelope};
