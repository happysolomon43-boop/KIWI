'use strict';

const test = require('node:test');
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');

const source = fs.readFileSync(path.join(__dirname, '..', '..', 'index.js'), 'utf8');

test('Phase 4 leaves 20 legacy calls in shadow mode and moves 7 IP tasks live', () => {
  const legacyCalls = source.match(/geminiModel\.generateContent\s*\(/g) || [];
  const taskIds = source.match(/taskId\s*:/g) || [];
  const liveCalls = source.match(/\bai\.run\s*\(/g) || [];

  assert.equal(legacyCalls.length, 20);
  assert.equal(taskIds.length, 20);
  assert.equal(liveCalls.length, 7);
  assert.match(source, /_aiRuntime\.observeLegacy\(taskId, _modelName\)/);
});

test('normal and Reckoning CBT remain distinguished in shadow routing', () => {
  assert.match(
    source,
    /ai_task_id:\s*isReckoningExam\s*\?\s*['"]RECKONING_CBT['"]\s*:\s*['"]MAIN_CBT['"]/
  );
  assert.match(source, /const _taskId\s*=\s*_opts\.ai_task_id\s*\|\|\s*['"]MAIN_CBT['"]/);
});

test('remaining legacy features retain canonical shadow task IDs', () => {
  for (const taskId of [
    'CBT_COMPLETION',
    'FLASHCARD_GENERATION',
    'IMPORT_IMAGE_EXTRACTION',
    'STUDY_TASK_GENERATION',
    'CONCEPT_CLUSTERING',
    'WEEKLY_CHRONICLE',
    'WEEKLY_PERSONA',
    'WEEKLY_ANCHOR',
    'MORNING_BRIEF',
    'DAILY_INVITATIONS',
    'BUBBLE_ADVISORY',
    'PRESSURE_EXPLANATION',
    'DEEP_AUDIT',
    'QUICK_QUESTIONS',
    'EXAM_DEBRIEF',
    'LIVING_PERSONA',
    'RECKONING_DEBRIEF',
    'LIVING_ACHIEVEMENTS',
  ]) {
    assert.match(source, new RegExp(`taskId\\s*:\\s*['"]${taskId}['"]`), taskId);
  }
});

test('all seven IP tasks execute through ai.run and no longer call the legacy wrapper', () => {
  for (const taskId of [
    'CARD_EXPLANATION',
    'RECLASSIFICATION_ALERT',
    'MASTERY_MOMENT',
    'ZONE_DESCRIPTION',
    'HIDDEN_DISCOVERY',
    'RETURN_GREETING',
    'CHRONICLE_ARTIFACT',
  ]) {
    assert.match(source, new RegExp(`ai\\.run\\(['"]${taskId}['"]`), taskId);
    assert.doesNotMatch(source, new RegExp(`taskId\\s*:\\s*['"]${taskId}['"]`), taskId);
  }
});
