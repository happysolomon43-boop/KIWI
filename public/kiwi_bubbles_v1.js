(function () {
  ...
  async function getBubbleGateState() {
    if (typeof isDemoMode === 'function' && isDemoMode()) {
      return { unlocked: true, current: BUBBLE_REQUIRED_SESSIONS, required: BUBBLE_REQUIRED_SESSIONS, demo: true };
    }
  ...
  async function renderBubbleHub() {
    ...
    try {
      ...
      main.innerHTML = `
        <div class="page-wrap page-enter bubble-hub-page">
          <div class="section-header bubble-hub-header">
            ...
            <button class="btn btn-primary" id="bubbleHubCreateBtn">${gate.unlocked ? 'Create Exam Goal' : 'Locked · ' + gate.current + '/' + gate.required + ' sessions'}</button>
          </div>
          ...
          ${!gate.unlocked ? `
            ...
            <section class="bubble-unlock-card is-unlocked">
              <div class="bubble-unlock-icon">🌳</div>
              <div>
                <div class="bubble-unlock-title">Mastery Bubbles are unlocked</div>
                <div class="bubble-unlock-copy">Create an exam goal for any subject with cards. KIWI will handle trajectory, daily contracts, Rescue mode, and queue prioritisation automatically.</div>
              </div>
            </section>`}
      ...
      const create = () => gate.unlocked ? openBubbleCreateModal() : showBubbleGateModal(gate);
  ...
  async function appendBubbleDashboardCard() {
    ...
    try {
      const [gate, bubbleData] = await Promise.all([getBubbleGateState(), api.getBubbles()]);
      if (AppState?.currentPage !== 'dashboard') return;
