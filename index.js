...
// Fix #38: conditional fetch — only re-fetch when FieldValue.increment is used
...
if (hasIncrements) { const doc = await ref.get(); return { userId, ...doc.data() }; }
return { userId, ...updatePayload };
},
async leaderboard(limit = 20, offset = 0) {
// Fix #47: offset() charges reads for skipped docs — migrate to cursor pagination
// when page tokens are available; for now preserve offset but avoid in hot paths
...
// Fix #46: use count() aggregation for total + Firestore-native offset pagination
// (eliminates loading ALL cards into memory just to count and slice)
...
const snap = await q.orderBy('created_at').offset(offset).limit(limit).get();
...
// Fix #30: IN operator (up to 30 per chunk) replaces sequential per-deck queries
if (!deckIds || deckIds.length === 0) return [];
const all = [];
for (let i = 0; i < deckIds.length; i += 30) {
const chunk = deckIds.slice(i, i + 30);
const snap = await firestore
.collection('cards')
.where('user_id', '==', userId)
.where('deck_id', 'in', chunk)
.get();
...
// Fix #36: was silently truncating to 10 cards; Firestore v9 supports 30 in 'in'
...
await ref.update(payload);
if (hasIncrements) { const doc = await ref.get(); return { id, ...doc.data() }; }
return { id, ...payload };
},
async findMany(userId, filters = {}, { limit = 20, offset = 0 } = {}) {
// Fix #47: use count() for accurate total without fetching all skipped docs
...
q.count().get(),
q.orderBy('started_at', 'desc').offset(offset).limit(limit).get(),
]);
return { sessions: snap.docs.map((d) => ({ id: d.id, ...d.data() })), total: countSnap.data().count || 0 };
},
async findBySubject(userId, subjectId, sinceDate) {
// Fix #37: was silently truncating to 10 deck IDs; chunked to 30 now
...
// Fix #8: deterministic doc ID eliminates WHERE scan
const docId = `${userId}_${subjectId}`;
const doc = await firestore.collection('subject_stats').doc(docId).get();
return doc.exists ? { id: doc.id, ...doc.data() } : null;
},
async upsert(userId, subjectId, data) {
// Fix #9: deterministic doc ID + set(merge) eliminates read-then-write
const docId = `${userId}_${subjectId}`;
const payload = { user_id: userId, subject_id: subjectId, ...data, updated_at: new Date() };
await firestore.collection('subject_stats').doc(docId).set(payload, { merge: true });
...
// Fix #13: code is the natural doc ID — eliminate WHERE scan entirely
const ref = firestore.collection('achievements').doc(code);
const payload = { ...data, code, updated_at: new Date() };
await ref.set(payload, { merge: true });
_achievementsCache = null; // Fix #49: invalidate cache on write
return { id: code, ...payload };
},
async findAll() {
// Fix #49: load once and cache — achievements are static (seeded, rarely changed)
if (_achievementsCache) return _achievementsCache;
const snap = await firestore.collection('achievements').get();
_achievementsCache = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
...
// Fix #14: deterministic doc ID enables direct markShown lookup
...
// Fix #14: deterministic doc ID eliminates WHERE scan
const docId = `${userId}_${achievementId}`;
await firestore.collection('user_achievements').doc(docId).update({ shown_to_user: true }).catch(() => {});
return true;
},
},
// ── tasks ────────────────────────────────────────────────────────────────────
...
let q = firestore.collection('community_decks').where('is_public', '==', true);
...
// Fix #15: originalDeckId is the natural doc ID — eliminate WHERE scan
const payload = { ...data, original_deck_id: originalDeckId, updated_at: new Date() };
await firestore.collection('community_decks').doc(originalDeckId).set(payload, { merge: true });
...
// Fix #16: ${communityDeckId}_${userId} eliminates WHERE scan on every rating write
const docId = `${communityDeckId}_${userId}`;
const payload = { community_deck_id: communityDeckId, user_id: userId, rating, updated_at: new Date() };
await firestore.collection('community_ratings').doc(docId).set(payload, { merge: true });
return { id: docId, ...payload };
},
...
// Fix #1: deterministic doc ID eliminates collection scan
const docId = `${userId}_${cardId}`;
const doc = await firestore.collection('card_states').doc(docId).get();
return doc.exists ? { id: doc.id, ...doc.data() } : null;
},
async create(userId, cardId, data) {
// Fix #2: deterministic doc ID; Fix #39/40: include deck_id/subject_id; Fix #41: exclude weight
...
// Fix #3: deterministic doc ID eliminates WHERE scan before every update
const docId = `${userId}_${cardId}`;
const ref = firestore.collection('card_states').doc(docId);
const existing = await ref.get();
if (existing.exists) {
const payload = { ...data, last_evaluated_at: new Date(), updated_at: new Date() };
await ref.update(payload);
return { id: docId, ...existing.data(), ...payload };
}
...
brainPressure: {
async get(userId, subjectId) {
// Fix #10: deterministic doc ID eliminates WHERE scan
const docId = `${userId}_${subjectId}`;
const doc = await firestore.collection('brain_pressure').doc(docId).get();
return doc.exists ? { id: doc.id, ...doc.data() } : null;
},
async set(userId, subjectId, data) {
// Fix #11: deterministic doc ID + set(merge) eliminates read-then-write
const docId = `${userId}_${subjectId}`;
const payload = {
user_id: userId,
subject_id: subjectId,
...
await firestore.collection('brain_pressure').doc(docId).set(payload, { merge: true });
return { id: docId, ...payload };
},
async findByUser(userId) {
const snap = await firestore
...
almanacEntries: {
async create(userId, data) {
// Fix #17: ${userId}_${entry_code} enables direct unlock lookup
const entryCode = data.entry_code || firestore.collection('almanac_entries').doc().id;
...
// Fix #17: deterministic doc ID eliminates WHERE scan
const docId = `${userId}_${entryCode}`;
const ref = firestore.collection('almanac_entries').doc(docId);
const doc = await ref.get();
if (!doc.exists) return null;
const updatePayload = { unlocked: true, unlocked_at: new Date(), narrative, updated_at: new Date() };
await ref.update(updatePayload);
return { id: docId, ...doc.data(), ...updatePayload };
},
},
// ── user_persona ────────────────────────────────────────────────────────────
userPersona: {
...
const doc = await firestore.collection('user_persona').doc(docId).get();
if (doc.exists) return { id: doc.id, ...doc.data() };
// Fallback: query for most recent persona (backward compat pre-migration)
const snap = await firestore
...
async create(userId, data) {
// Fix #12: deterministic doc ID keyed to assigned week
...
// Fix #4: deterministic doc ID eliminates 3-field WHERE scan
const docId = `${userId}_${type}_${dateStr}`;
const doc = await firestore.collection('daily_ritual_cache').doc(docId).get();
return doc.exists ? { id: doc.id, ...doc.data() } : null;
},
async set(userId, type, dateStr, data) {
// Fix #5: deterministic doc ID + set(merge) eliminates read-then-write (2 ops → 1)
const docId = `${userId}_${type}_${dateStr}`;
const payload = { user_id: userId, type, date: dateStr, ...data, updated_at: new Date() };
await firestore.collection('daily_ritual_cache').doc(docId).set(payload, { merge: true });
return { id: docId, ...payload };
},
},
...
// Fix #6: deterministic doc ID eliminates WHERE scan
const docId = `${userId}_${itemCode}`;
const doc = await firestore.collection('user_inventory').doc(docId).get();
return doc.exists ? { id: doc.id, ...doc.data() } : null;
},
async setItem(userId, itemCode, data) {
// Fix #7: deterministic doc ID + set(merge) eliminates read-then-write
const docId = `${userId}_${itemCode}`;
const payload = {
user_id: userId,
item_code: itemCode,
...
await firestore.collection('user_inventory').doc(docId).set(payload, { merge: true });
return { id: docId, ...payload };
},
async findByUser(userId) {
const snap = await firestore
...
purchase_limit: 999, // unlimited per user (marketplace config, not a query limit)
},
{
...
Volume is a byproduct of coverage — never a target. Do not pad. Generate exactly the count requested.
...
let batchCount = 0;
for (const cardId of cardIds) {
if (existingMap.has(cardId)) {
results.push(existingMap.get(cardId));
continue;
}
const docId = `${userId}_${cardId}`;
const ref = firestore.collection('card_states').doc(docId);
...
if (batchCount % 499 === 0) await batch.commit();
}
...
const cardStates = [];
for (const card of allCards) {
let stateDoc = stateMap.get(card.id);
...
const created = await db.cards.createMany(req.user.id, deck_id, cardsData);
await db.decks.update(req.user.id, deck_id, { card_count: FieldValue.increment(created.length) });
...
const modifiedQueue    = await modifySessionQueueForBubbles(
  req.user.id, dueQueue, bubbleSubjectId
).catch(() => dueQueue);
...
...streakUpdates,
total_sessions_completed: FieldValue.increment(1),
total_study_minutes: FieldValue.increment(Math.round(durationSec / 60)),
});
...
try {
const [usersCountSnap, cardsCountSnap, sessionsCountSnap, statsList] = await Promise.all([
firestore.collection('users').count().get(),
firestore.collection('cards').count().get(),
firestore.collection('sessions').count().get(),
db.userStats.findAll(),
]);
const total_users    = usersCountSnap.data().count    || 0;
const total_cards    = cardsCountSnap.data().count    || 0;
const totalSessions  = sessionsCountSnap.data().count || 0;
res.json({
total_users,
total_cards,
...
console.log(`[KIWI] ✅ Achievements, marketplace, and community decks seeded`);
    ...
    async function sendBrevoEmail(toEmail, templateName, params = {}) {
      if (_brevoAccounts.length === 0) {
        console.warn(`[KIWI] No Brevo API keys configured. Would have sent "${templateName}" to ${toEmail}. Set BREVO_API_KEY on Render to enable emails.`);
        return false;
      }
      const templates = {
        welcome: {
...
<p><a href="${process.env.APP_URL || 'https://kiwi.app'}" style="background:#2d6a4f;color:#dff2eb;padding:10px 20px;border-radius:8px;text-decoration:none;display:inline-block;margin-top:8px;">Open KIWI →</a></p>
